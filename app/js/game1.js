...
function adduser(data){
    ...
    var html='<div class="member member'+indexuser+'" id="user'+data.user.id+'">'
	 ...
	 html += '<div style="position: relative;"><div style="position: absolute; z-index: 880; display: none;" class="watchzt" ><img src="/app/watchzt.png"/ style="width: 100%;margin-top:1.0vh;margin-left:1.3vw "></div> <img src="' + data.user.img + '" class="avatar" /></div>',
...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillText("房间号:"+sj.id, 185,285);
        ctx.fillText(time1, 375, 285);
        ctx.fillText(sj.zjs+"局", 565, 285);
        ...
        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ...
                ctx.fillRect(142,575-(0-(i-2)*103),519,96);
            }
            var user=sj['user'][i];
            ctx.font = "35px 微软雅黑";
            ctx.fillStyle = "#d8cb66";
            //ctx.fillText(user.id, 195, 380+(i*75));
            ctx.fillText(decode64(user.nickname).substring(0,9), 225, 435+(i*100));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(decode64(user.nickname).substring(0,9), 225, 435+(i*100));
                ctx.fillText(user.dqjf, 530, 435+(i*100));
            }
            ctx.fillText( user.dqjf, 530, 435+(i*100));
        }
...
function operationButton(data){
    ...
    if(data=='7'){
        ...
        html+='<img class="operationButton-gg" src="/app/img/bull9/bull_button_blue.png"  /> ';
        html+='<div class="operationButton-gg1" style="position: absolute; top: 0px; left: 0px; width: 100%; height: 100%;line-height: 9vw;font-size: 4.9vw;text-align: center;color:#fff;font-weight: bold;">';
        html+='摊牌';
        html+='</div>';
        html+='</div>';
    }
