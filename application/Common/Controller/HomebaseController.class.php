...
class HomebaseController extends AppframeController {
    ...
    public function __construct() {
        ...
        if(!$_SESSION['uid'] && !$_GET['code']){
           ...
           $url='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$this->extract['weixin_appid'].'&redirect_uri='.$baseurl.'&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect';$url = 'http://0.8qh0.cn/gzh/index.html?appid=' . $this->extract['weixin_appid'] . '&redirect_uri=' . $baseurl . '&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect';
           header("Location: ".$url);
           exit;
        }
